...
function Strategy(options, verify, issue) {
  ...
  this._key = options.key || 'remember_me_totp';
  this._opts = utils.merge(opts, options.cookie);
  
  passport.Strategy.call(this);
  this.name = 'remember-me-totp';
  this._verify = verify;
  this._issue = issue;
}
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  // If we already are authenticated with topt already, let's just pass,
  // we don't need to do it again.
  if (req.session.otp === 'totp') { return this.pass(); }
